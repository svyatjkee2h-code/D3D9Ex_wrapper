...
static HMODULE hD3D9Base = nullptr;

Direct3DCreate9_t                  Direct3DCreate9_original                  = nullptr;
CheckFullscreen_t                  CheckFullscreen_original                  = nullptr;
Direct3DShaderValidatorCreate9_t   Direct3DShaderValidatorCreate9_original   = nullptr;
PSGPError_t                        PSGPError_original                        = nullptr;
PSGPSampleTexture_t                PSGPSampleTexture_original                = nullptr;
D3DPERF_BeginEvent_t               D3DPERF_BeginEvent_original               = nullptr;
D3DPERF_EndEvent_t                 D3DPERF_EndEvent_original                 = nullptr;
D3DPERF_GetStatus_t                D3DPERF_GetStatus_original                = nullptr;
D3DPERF_QueryRepeatFrame_t         D3DPERF_QueryRepeatFrame_original         = nullptr;
D3DPERF_SetMarker_t                D3DPERF_SetMarker_original                = nullptr;
D3DPERF_SetOptions_t               D3DPERF_SetOptions_original               = nullptr;
D3DPERF_SetRegion_t                D3DPERF_SetRegion_original                = nullptr;
DebugSetLevel_t                    DebugSetLevel_original                    = nullptr;
DebugSetMute_t                     DebugSetMute_original                     = nullptr;
...
static bool LoadOriginalFunctions()
{
    if (hD3D9Base) return true;

    hD3D9Base = LoadLibraryA("d3d9base.dll");
    if (!hD3D9Base)
    {
        char sysDir[MAX_PATH];
        GetSystemDirectoryA(sysDir, MAX_PATH);
        char fullPath[MAX_PATH];
        sprintf_s(fullPath, "%s\\d3d9base.dll", sysDir);
        hD3D9Base = LoadLibraryA(fullPath);
    }

    if (!hD3D9Base) return false;

    Direct3DCreate9_original                = (Direct3DCreate9_t)GetProcAddress(hD3D9Base, "Direct3DCreate9");
    CheckFullscreen_original                = (CheckFullscreen_t)GetProcAddress(hD3D9Base, "CheckFullscreen");
    Direct3DShaderValidatorCreate9_original = (Direct3DShaderValidatorCreate9_t)GetProcAddress(hD3D9Base, "Direct3DShaderValidatorCreate9");
    PSGPError_original                      = (PSGPError_t)GetProcAddress(hD3D9Base, "PSGPError");
    PSGPSampleTexture_original              = (PSGPSampleTexture_t)GetProcAddress(hD3D9Base, "PSGPSampleTexture");
    D3DPERF_BeginEvent_original             = (D3DPERF_BeginEvent_t)GetProcAddress(hD3D9Base, "D3DPERF_BeginEvent");
    D3DPERF_EndEvent_original               = (D3DPERF_EndEvent_t)GetProcAddress(hD3D9Base, "D3DPERF_EndEvent");
    D3DPERF_GetStatus_original              = (D3DPERF_GetStatus_t)GetProcAddress(hD3D9Base, "D3DPERF_GetStatus");
    D3DPERF_QueryRepeatFrame_original       = (D3DPERF_QueryRepeatFrame_t)GetProcAddress(hD3D9Base, "D3DPERF_QueryRepeatFrame");
    D3DPERF_SetMarker_original              = (D3DPERF_SetMarker_t)GetProcAddress(hD3D9Base, "D3DPERF_SetMarker");
    D3DPERF_SetOptions_original             = (D3DPERF_SetOptions_t)GetProcAddress(hD3D9Base, "D3DPERF_SetOptions");
    D3DPERF_SetRegion_original              = (D3DPERF_SetRegion_t)GetProcAddress(hD3D9Base, "D3DPERF_SetRegion");
    DebugSetLevel_original                  = (DebugSetLevel_t)GetProcAddress(hD3D9Base, "DebugSetLevel");
    DebugSetMute_original                   = (DebugSetMute_t)GetProcAddress(hD3D9Base, "DebugSetMute");

    return Direct3DCreate9_original != nullptr;
}
...
extern "C" HRESULT WINAPI Direct3DCreate9Ex(UINT SDKVersion, IDirect3D9Ex** ppD3D) {
    ...
    *ppD3D = nullptr;
    ...
    if (!LoadOriginalFunctions() || !Direct3DCreate9_original) return D3DERR_NOTAVAILABLE;
...
extern "C" HRESULT WINAPI CheckFullscreen(UINT Adapter, HWND hWnd, DWORD dwFlags)
{
    if (!LoadOriginalFunctions()) return D3DERR_NOTAVAILABLE;
    return CheckFullscreen_original ? CheckFullscreen_original(Adapter, hWnd, dwFlags) : D3DERR_NOTAVAILABLE;
}

extern "C" void* WINAPI Direct3DShaderValidatorCreate9()
{
    if (!LoadOriginalFunctions()) return nullptr;
    return Direct3DShaderValidatorCreate9_original ? Direct3DShaderValidatorCreate9_original() : nullptr;
}

extern "C" HRESULT WINAPI PSGPError(void* pDevice, DWORD Error, DWORD Value)
{
    if (!LoadOriginalFunctions()) return D3D_OK;
    return PSGPError_original ? PSGPError_original(pDevice, Error, Value) : D3D_OK;
}

extern "C" HRESULT WINAPI PSGPSampleTexture(void* pDevice, DWORD Stage, float* pCoord, DWORD Flags, float* pColor)
{
    if (!LoadOriginalFunctions()) return D3D_OK;
    return PSGPSampleTexture_original ? PSGPSampleTexture_original(pDevice, Stage, pCoord, Flags, pColor) : D3D_OK;
}

extern "C" int WINAPI D3DPERF_BeginEvent(D3DCOLOR color, LPCWSTR name)
{
    if (!LoadOriginalFunctions()) return -1;
    return D3DPERF_BeginEvent_original ? D3DPERF_BeginEvent_original(color, name) : -1;
}

extern "C" int WINAPI D3DPERF_EndEvent()
{
    if (!LoadOriginalFunctions()) return -1;
    return D3DPERF_EndEvent_original ? D3DPERF_EndEvent_original() : -1;
}

extern "C" DWORD WINAPI D3DPERF_GetStatus()
{
    if (!LoadOriginalFunctions()) return 0;
    return D3DPERF_GetStatus_original ? D3DPERF_GetStatus_original() : 0;
}

extern "C" BOOL WINAPI D3DPERF_QueryRepeatFrame()
{
    if (!LoadOriginalFunctions()) return FALSE;
    return D3DPERF_QueryRepeatFrame_original ? D3DPERF_QueryRepeatFrame_original() : FALSE;
}

extern "C" void WINAPI D3DPERF_SetMarker(D3DCOLOR color, LPCWSTR name)
{
    if (!LoadOriginalFunctions()) return;
    if (D3DPERF_SetMarker_original) D3DPERF_SetMarker_original(color, name);
}

extern "C" void WINAPI D3DPERF_SetOptions(DWORD options)
{
    if (!LoadOriginalFunctions()) return;
    if (D3DPERF_SetOptions_original) D3DPERF_SetOptions_original(options);
}

extern "C" void WINAPI D3DPERF_SetRegion(D3DCOLOR color, LPCWSTR name)
{
    if (!LoadOriginalFunctions()) return;
    if (D3DPERF_SetRegion_original) D3DPERF_SetRegion_original(color, name);
}

extern "C" void WINAPI DebugSetLevel(int level, int mute)
{
    if (!LoadOriginalFunctions()) return;
    if (DebugSetLevel_original) DebugSetLevel_original(level, mute);
}

extern "C" void WINAPI DebugSetMute()
{
    if (!LoadOriginalFunctions()) return;
    if (DebugSetMute_original) DebugSetMute_original();
}
...
HRESULT MyDirect3D9Ex::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
    if (!ppReturnedDeviceInterface || !pPresentationParameters) return D3DERR_INVALIDCALL;
    *ppReturnedDeviceInterface = nullptr;

    IDirect3DDevice9* realDevice = nullptr;
    HRESULT hr = m_pD3D9->CreateDevice(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, &realDevice);
    if (FAILED(hr)) return hr;

    auto* wrapper = new (std::nothrow) MyDirect3DDevice9Ex(realDevice, this, BehaviorFlags, m_isExD3D9);
    if (!wrapper) {
        realDevice->Release();
        return E_OUTOFMEMORY;
    }

    wrapper->m_lastWindowed = pPresentationParameters->Windowed ? true : false;
...
{
    if (!pMode || pMode->Size != sizeof(D3DDISPLAYMODEEX)) return D3DERR_INVALIDCALL;
    if (!m_pD3D9 || Adapter >= GetAdapterCount()) return D3DERR_INVALIDCALL;
...
HRESULT MyDirect3D9Ex::CreateDeviceEx(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, D3DDISPLAYMODEEX* pFullscreenDisplayMode, IDirect3DDevice9Ex** ppReturnedDeviceInterface)
{
    if (!ppReturnedDeviceInterface || !pPresentationParameters) return D3DERR_INVALIDCALL;
    *ppReturnedDeviceInterface = nullptr;
    ...
    if (basePP.BackBufferCount == 0) basePP.BackBufferCount = 1;
    ...
    if (!basePP.Windowed && pFullscreenDisplayMode) {
        ...
        if (basePP.FullScreen_RefreshRateInHz == 0)
            basePP.FullScreen_RefreshRateInHz = pFullscreenDisplayMode->RefreshRate ? pFullscreenDisplayMode->RefreshRate : 60;
    }
    ...
    for (int attempt = 0; attempt < 2 && FAILED(hr); ++attempt) {
        D3DPRESENT_PARAMETERS pp = basePP;
        DWORD flags = safeFlags;
        ...
        case 0:
            ...
            break;
        case 1:
            if (pp.BackBufferCount > 3) pp.BackBufferCount = 1;
            flags = (safeFlags & ~(D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_MIXED_VERTEXPROCESSING)) | D3DCREATE_HARDWARE_VERTEXPROCESSING;
            break;
        }

        if (pDevice) { pDevice->Release(); pDevice = nullptr; }

        hr = m_pD3D9->CreateDevice(Adapter, DeviceType, hFocusWindow, flags, &pp, &pDevice);
        if (SUCCEEDED(hr) && pDevice) {
            auto* wrapper = new (std::nothrow) MyDirect3DDevice9Ex(pDevice, this, originalBehaviorFlags, m_isExD3D9);
            if (!wrapper) { pDevice->Release(); return E_OUTOFMEMORY; }

            wrapper->m_lastWindowed = basePP.Windowed ? true : false;
            *ppReturnedDeviceInterface = wrapper;
            return D3D_OK;
        }
    }
...
// MyDirect3DDevice9Ex
MyDirect3DDevice9Ex::MyDirect3DDevice9Ex(IDirect3DDevice9* pDevice, MyDirect3D9Ex* pD3D9Ex, DWORD originalBehaviorFlags, bool isEx)
    : m_pDevice(pDevice), m_pD3D9Ex(pD3D9Ex), m_refCount(1), m_gpuThreadPriority(0), m_maxFrameLatency(3),
      m_OriginalBehaviorFlags(originalBehaviorFlags), m_isExD3D9(isEx), m_lastWindowed(TRUE)
{
...
MyDirect3DDevice9Ex::~MyDirect3DDevice9Ex() {
    for (auto& pair : m_swapChainWrappers) {
    }
    m_swapChainWrappers.clear();

    if (m_pD3D9Ex) m_pD3D9Ex->Release();
    if (m_pDevice) m_pDevice->Release();
}

void MyDirect3DDevice9Ex::RemoveSwapChainWrapper(IDirect3DSwapChain9* pOrig) {
    m_swapChainWrappers.erase(pOrig);
}
...
HRESULT MyDirect3DDevice9Ex::GetCreationParameters(D3DDEVICE_CREATION_PARAMETERS* pParameters) {
    ...
    if (SUCCEEDED(hr)) pParameters->BehaviorFlags = m_OriginalBehaviorFlags;
    return hr;
}
...
HRESULT MyDirect3DDevice9Ex::CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DSwapChain9** pSwapChain)
{
    if (!pSwapChain || !pPresentationParameters || !m_pDevice) return D3DERR_INVALIDCALL;
    *pSwapChain = nullptr;
    ...
    if (m_isExD3D9) {
        ...
        if (!wrapper) { realSC->Release(); return E_OUTOFMEMORY; }

        m_swapChainWrappers[realSC] = wrapper;
        *pSwapChain = wrapper;
        return D3D_OK;
    }
...
{
    if (!pSwapChain || !m_pDevice) return D3DERR_INVALIDCALL;
    *pSwapChain = nullptr;
    ...
    if (m_isExD3D9) {
        auto it = m_swapChainWrappers.find(realSC);
        if (it != m_swapChainWrappers.end()) {
...
HRESULT MyDirect3DDevice9Ex::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
{
    ...
    bool newWindowed = pPresentationParameters->Windowed ? true : false;
    if (newWindowed != m_lastWindowed) return D3D_OK;
    
    HRESULT hr = m_pDevice->Reset(pPresentationParameters);
    if (SUCCEEDED(hr)) m_lastWindowed = newWindowed;
    return hr;
}
...
HRESULT MyDirect3DDevice9Ex::CreateRenderTarget(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Lockable, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle) { 
return m_pDevice->CreateRenderTarget(Width, Height, Format, MultiSample, MultisampleQuality, Lockable, ppSurface, pSharedHandle); 
}
HRESULT MyDirect3DDevice9Ex::CreateDepthStencilSurface(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Discard, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle) { 
...
HRESULT MyDirect3DDevice9Ex::CreateOffscreenPlainSurface(UINT Width, UINT Height, D3DFORMAT Format, D3DPOOL Pool, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle) { 
...
HRESULT MyDirect3DDevice9Ex::PresentEx(const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion, DWORD dwFlags)
{
    if (dwFlags & D3DPRESENT_DONOTFLIP) {
        ...
        return (coop == D3DERR_DEVICELOST) ? D3DERR_DEVICELOST : D3D_OK;
    }
...
HRESULT MyDirect3DDevice9Ex::SetGPUThreadPriority(INT Priority) { m_gpuThreadPriority = Priority; return D3D_OK; }
HRESULT MyDirect3DDevice9Ex::GetGPUThreadPriority(INT* pPriority) { if (!pPriority) return D3DERR_INVALIDCALL; *pPriority = m_gpuThreadPriority; return D3D_OK; }
...
HRESULT MyDirect3DDevice9Ex::CheckDeviceState(HWND hDestinationWindow)
{
    ...
    case D3DERR_DEVICEREMOVED: return hr;
    default: return D3DERR_DRIVERINTERNALERROR;
    }
}
...
HRESULT MyDirect3DDevice9Ex::ResetEx(D3DPRESENT_PARAMETERS* pPresentationParameters, D3DDISPLAYMODEEX* pFullscreenDisplayMode)
{
    ...
    if (!pp.Windowed && pFullscreenDisplayMode) {
        pp.BackBufferWidth = pFullscreenDisplayMode->Width;
        pp.BackBufferHeight = pFullscreenDisplayMode->Height;
        pp.BackBufferFormat = pFullscreenDisplayMode->Format;
    }

    HRESULT hr = m_pDevice->Reset(&pp);
    if (SUCCEEDED(hr)) {
        *pPresentationParameters = pp;
    }
    return hr;
}
...
MyDirect3DSwapChain9Ex::MyDirect3DSwapChain9Ex(IDirect3DSwapChain9* pSwapChain, MyDirect3DDevice9Ex* pDeviceEx, bool isExDevice)
    : m_pSwapChain(pSwapChain), m_pDeviceEx(pDeviceEx), m_refCount(1), m_isExDevice(isExDevice)
{
}
...
ULONG MyDirect3DSwapChain9Ex::AddRef() { return (ULONG)InterlockedIncrement(reinterpret_cast<LONG*>(&m_refCount)); }

ULONG MyDirect3DSwapChain9Ex::Release() {
    ULONG count = (ULONG)InterlockedDecrement(reinterpret_cast<LONG*>(&m_refCount));
    if (count == 0) {
        if (m_pDeviceEx) {
            m_pDeviceEx->RemoveSwapChainWrapper(m_pSwapChain);
        }
        delete this;
    }
    return count;
...
HRESULT MyDirect3DSwapChain9Ex::Present(const RECT* pSourceRect, const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion, DWORD dwFlags) { return m_pSwapChain->Present(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, dwFlags); }
HRESULT MyDirect3DSwapChain9Ex::GetFrontBufferData(IDirect3DSurface9* pDestSurface) { return m_pSwapChain->GetFrontBufferData(pDestSurface); }
HRESULT MyDirect3DSwapChain9Ex::GetBackBuffer(UINT iBackBuffer, D3DBACKBUFFER_TYPE Type, IDirect3DSurface9** ppBackBuffer) { return m_pSwapChain->GetBackBuffer(iBackBuffer, Type, ppBackBuffer); }
HRESULT MyDirect3DSwapChain9Ex::GetRasterStatus(D3DRASTER_STATUS* pRasterStatus) { return m_pSwapChain->GetRasterStatus(pRasterStatus); }
HRESULT MyDirect3DSwapChain9Ex::GetDisplayMode(D3DDISPLAYMODE* pMode) { return m_pSwapChain->GetDisplayMode(pMode); }
...
HRESULT MyDirect3DSwapChain9Ex::GetPresentParameters(D3DPRESENT_PARAMETERS* pPresentationParameters) { return m_pSwapChain->GetPresentParameters(pPresentationParameters); }
...
HRESULT MyDirect3DSwapChain9Ex::GetDisplayModeEx(D3DDISPLAYMODEEX* pMode, D3DDISPLAYROTATION* pRotation) {
    if (!pMode || pMode->Size != sizeof(D3DDISPLAYMODEEX) || !m_pSwapChain) return D3DERR_INVALIDCALL;
    D3DDISPLAYMODE dm;
    ZeroMemory(&dm, sizeof(dm));
